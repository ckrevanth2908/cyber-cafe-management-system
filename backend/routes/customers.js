...
router.get('/', (req, res) => {
  try {
    ...
    return res.json(db.prepare('SELECT * FROM customers ORDER BY id DESC').all());
  } catch (err) {
...
router.post('/', (req, res) => {
  try {
    ...
    const result = db.prepare(`
      INSERT INTO customers (name, age, address, phone, email) VALUES (?, ?, ?, ?, ?)
...
router.get('/:id', (req, res) => {
  try {
    ...
    const printTransactions = db.prepare(`
      ...
      WHERE pt.customer_id = ? ORDER BY pt.id DESC
    `).all(req.params.id);
    return res.json({ ...customer, sessions, print_transactions: printTransactions });
  } catch (err) {
...
router.put('/:id', (req, res) => {
  try {
    ...
    db.prepare(`
      UPDATE customers
      SET name = COALESCE(?, name), age = COALESCE(?, age), address = COALESCE(?, address),
          phone = COALESCE(?, phone), email = COALESCE(?, email)
      WHERE id = ?
    `).run(name, age ? parseInt(age, 10) : null, address, phone, email, req.params.id);
    return res.json(db.prepare('SELECT * FROM customers WHERE id = ?').get(req.params.id));
  } catch (err) {
