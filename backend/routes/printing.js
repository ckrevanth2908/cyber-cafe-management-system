...
function recordRevenue(serviceType, amount) {
  ...
  if (!record) {
    ...
    record = db.prepare('SELECT * FROM daily_revenue WHERE date = ?').get(today);
  }
...
router.post('/', (req, res) => {
  ...
  if (!rateRow) {
    return res.status(400).json({ detail: 'Invalid service type rate' });
  }
  ...
  // Record into daily revenue automatically
...
router.get('/:id', (req, res) => {
  ...
  if (!tx) {
    return res.status(404).json({ detail: 'Printing transaction not found' });
  }
  return res.json(tx);
});
